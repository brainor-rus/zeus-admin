    ...
    $( ".sortable" ).sortable({
        ...
        update: function( event, ui ) {//стартует и при изменении уровня списка и при изменении положения в списке
            if (this === ui.item.parent()[0]){//стартуем 1 раз
                ...
                var model = $(this).parent().data("modelPath");
                var prev_id = $(ui.item).prev('li').attr('id');
                var next_id = $(ui.item).next('li').attr('id');
                console.log(id +'-'+ parent_id +'-'+ prev_id+'-'+ next_id+'-'+model);
                $.ajaxSetup({
                    headers: {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/' + window.adminUrl + '/api/tree-elements/category/reorder',
                    data: {id:id,parent_id:parent_id,prev_id:prev_id,next_id:next_id,model:model},
                    cache: false,
                    beforeSend: function() {
