...
use Zeus\Admin\Plugins\BrainorCommerce\Models\ZeusAdminAttributeName;
use Zeus\Admin\Plugins\BrainorCommerce\Models\ZeusAdminOffer;
...
class ZeusAdminOffers extends Section
{
    protected $model = 'Zeus\Admin\Plugins\BrainorCommerce\Models\ZeusAdminOffer';
...
//            Column::text('producer.name', 'Производитель'),
    ...
    public static function onEdit($id)
    {
        ...
        $attributes = null;
        if($id) {
            $offer = ZeusAdminOffer::where('id', $id)->first();
            $attributes = ZeusAdminAttributeName::where('category_id', $offer->category_id)
    ...
    {
        // Атрибуты
        DB::table('zeus_admin_attribute_values')->where('offer_id', $model->id)->delete();
        if(isset($_POST['attributes'])){
            foreach ($_POST['attributes'] as $attribute){
                $attribute_name_check = ZeusAdminAttributeName::where('name', trim($attribute['name']))->where('category_id', $_POST['category_id'])->first();
                if(!isset($attribute_name_check)){
                    $attributeName = new ZeusAdminAttributeName;
                ...
                DB::table('zeus_admin_attribute_values')->insert(
                    [
